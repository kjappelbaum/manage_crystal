...
def parse_mol_coords(coord_l: list):
    x = []
    y = []
    z = []
    atom_types = []
    ...
    for line in coord_l:
        ...
        atom_types.append(splitted[4])
        charges.append(float(splitted[5]))

    return x, y, z, atom_types, charges


def parse_mol(file):
    """Experimental. I do not know the file specifications. Using structure from
    The ones in RASPA and MUSIC
    """
    c = Crys()
    ...
    coord_type = None
    cart_line = None
    coord_info_line = None
    for i, line in enumerate(lines):
        if "Coord_Info" in line:
            coord_info_line = i
            if "Cartesian" in line:
                coord_type = "Cartesian"
            else:
                coord_type = "Frac"
        elif "Fundcell_Info" in line:
            cart_line = i

    x, y, z, atom_types, charges = parse_mol_coords(
        lines[coord_info_line + 2 : cart_line - 1]
    )

    c.length[0], c.length[1], c.length[2] = [
        float(i) for i in lines[cart_line + 1].split()
    ]
    c.angle_deg[0], c.angle_deg[1], c.angle_deg[2] = [
        float(i) for i in lines[cart_line + 2].split()
    ]

    _coord = np.zeros((len(x), 3))
    _coord[:, 0] = x
    _coord[:, 1] = y
    _coord[:, 2] = z
    ...
    c.atom_element = atom_types
    c.atom_type = atom_types

    if charges:
        c.atom_charge = charges
...
def parse_cp2k(file):
    ...
    while True:
        data = file.readline().split()
        if data[0] == "SCALED" and (
            len(data) == 1 or data[1].lower() in ["t", "true", ".true."]
        ):
            scaled_coord = True
